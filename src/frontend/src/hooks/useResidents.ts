...
import type { HostelData, PaymentRecord, Resident } from "../types";
...
const SAMPLE_PAYMENT_RECORDS: PaymentRecord[] = [
  {
    id: "p1",
    residentId: "r1",
    month: 3,
...
export function useResidents() {
  ...
  const updateResident = useCallback(
    (id: string, updates: Partial<Omit<Resident, "id" | "createdAt">>) => {
  ...
  const getResident = useCallback(
    (id: string) => data.residents.find((r) => r.id === id) ?? null,
    [data],
  );
